...
class PickHelper {
  ...
  pick(normalizedPosition, scene, camera, time) {
    ...
    if (intersectedObjects.length) {
      ...
      this.molekulaDiv = document.createElement('div');
      this.molekulaDiv.className = 'label';
      this.molekulaDiv.textContent = 'Moon';

      this.molekulaDiv.style.backgroundColor = 'transparent';
      this.molekulaDiv.style.color = 'black'
      this.molekulaLabel = new CSS2DObject(this.molekulaDiv);
      this.molekulaLabel.position.set(1.5 * 0, 0, 0);
      this.molekulaLabel.center.set(0, 1);
      this.pickedObject.add(this.molekulaLabel);
      this.molekulaLabel.layers.set(0);
...
function clearPickPosition() {
  ...
  if (pickHelper && pickHelper.molekulaDiv)
  {
    console.log("clearPickPosition")
    pickHelper.molekulaDiv.remove();
    pickHelper.molekulaLabel.removeFromParent();
  }
}
